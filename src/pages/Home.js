...
class Home extends Component {
    ...
    componentDidMount() {
        this.setState({trials: this.props.results})
    }
    ...
    downloadPdf = () => {
        ...
        const docDefinition = {
            ...
            watermark: { text: 'Tempus', color: 'black', opacity: 0.2, bold: true, italics: false },
            content: [
                {
                ...
                {text: 'Saved Trials Report', style: 'title'},
                {
                    table: {
    ...
    submitSearch = event => {
        ...
        API.getTrials(topic, perPage).then(res => {
            ...
            this.props.updateAppResults(this.state.trials))
        })
    };
            ...
            <div className="App">
                ...
                <div className="panel panel-default">
                    ...
                    <div className="panel-body container">
                        <button className="btn btn-info" onClick={this.downloadPdf}>Download PDF</button>
                    ...
                    <div className="panel-body">
                        { this.state.trials.length > 0 ? 
                            this.state.trials.map((a, i) => (
                                <div key={i}>
                                    <Result removeTrial={this.removeTrial} saveTrial={this.saveTrial} resultObject={a} />
                                </div>))
                            : 
                ...
                <div id='divToDownload' className="panel panel-default" >
                    ...
                    <div id="customers">
                        <table id="tab_customers" className="table table-striped" >
                            ...
                            <tbody>
                                {this.state.trials.map((a, i) => (
                                    <tr key={i}>
                                        <td>{a.public_title}</td>
                                        <td>{!a.interventions ? 'No interventions provided' : a.interventions.length === 0 ? 'No interventions provided' : <ul> {a.interventions.map((el, i) => <li key={i}>{el.name}</li>)} </ul>}</td>
                                        <td>{!a.locations ? 'No locations provided' : a.locations.length === 0 ? 'No locations provided' : <ul> {a.locations.map((el, i) => <li key={i}>{el.name}</li>)} </ul>}</td>
                                    </tr>))}
                            </tbody>
                        </table>
                    </div>
